...
```
L7  PRESENTATION    machinery yellow + hi-vis green (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      planthire-ai-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: UK plant-hire, machinery categories, CPCS operator cards, daily rates
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "planthire"
...
```bash
git clone https://github.com/CSOAI-ORG/planthire-hive
cd planthire-hive
...
`https://planthire.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
8-15% marketplace fees (equipment values $1k-10k/day).
